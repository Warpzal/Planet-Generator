...
document.getElementById('model_checker').innerHTML =
    'Please wait for the model to load. (This can take up to 20 seconds)'
...
function erase() {
    const { ctx, width, height } = getCurrentCanvas()
...
async function generateCustom() {
    ...
    let inputtensor = tf.randomNormal([1, 100], input / 100, 0.2)
    console.log(`Predicting: ${inputtensor}`)

    // Prediction
    let outputtensor = await generator.predict(inputtensor)

    outputtensor = outputtensor.reshape([width, height, 3])
    ...
    ctx.putImageData(
        new ImageData(Uint8ClampedArray.from(result), width, width),
        0,
        0
    )
...
async function generateRandom() {
    ...
    document.getElementById('slider').value =
        inputtensor.mean().dataSync()[0] * 100
    document.getElementById('output').innerHTML =
        'Mean of Z: ' + Math.round(inputtensor.mean().dataSync()[0] * 100) / 100
    ...
    ctx.putImageData(
        new ImageData(Uint8ClampedArray.from(result), 500, 500),
        0,
        0
    )
